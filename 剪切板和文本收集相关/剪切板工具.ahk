...
cl:=ini("clip.ini")
if (cl.clip.合并)
Hotkey,% cl.clip.合并, 合并
if (cl.clip.逆序)
Hotkey,% cl.clip.逆序, 逆序	
if (cl.clip.打开窗口)
Hotkey,% cl.clip.打开窗口, 打开窗口	
if (cl.clip.额外复制快捷键)
Hotkey,% cl.clip.额外复制快捷键,额外复制快捷键
...
打开窗口:
if (show==1){
  WinHide, 剪切板工具 ahk_class AutoHotkeyGUI
  show:=0
	SetTimer,悬浮显示完整,off
}else if (show==0){
  gui,剪切板工具:Show,NoActivate,剪切板工具
SetTimer,悬浮显示完整
Show:=1
}
return
...
{ 
  ...
  if (Cmd="Click" && 编辑模式=1) ;编辑器打开
  {
    i:=SubStr(A_GuiControl, 3)
    if (i>=1 and i<=15)
    {
      s:=MyClipData[i:=(page-1)*15+i]
      if (s="")
        return
    ; 将s变量的内容写入test.txt文件
    FileDelete, C:\test.txt ; 删除之前的文件
    FileAppend, %s%, C:\test.txt ; 将内容写入文件

    ; 调用编辑器打开 test.txt
    RunWait,%编辑器% C:\test.txt
    MsgBox,编辑好了后点掉这个msg
    ; 读取 test.txt 文件内容并插入到 MyClipData 数组的第1个
    FileRead, s, C:\test.txt
  ...
  if (Cmd="Click" && 编辑模式!=1)
  {
    ...
    if (i>=1 and i<=15)
    {
      ...
      step:=-1            ;连续粘贴回到初始位置
      if (MyClipData.length()<(page-1)*15+1)
        page--
    }
    else if (i=16 and page>1)
      page--
    else if (i=19 and MyClipData.length()>page*15)
      page++
    else if (i=17)
      clear:=!clear
    else if(i=18)
  ...
  if !IsObject(MyClipData)
  {
    ...
    Gui, 剪切板工具: Destroy
    ; Gui, 剪切板工具: +AlwaysOnTop +ToolWindow +E0x08000000
    Gui, 剪切板工具: +AlwaysOnTop -DPIScale -MinimizeBox -MaximizeBox +E0x08000000
    ; Gui, 剪切板工具: +E0x08000000
    Gui, 剪切板工具: Margin, 15, 15
    Gui, 剪切板工具: Color, DDEEFF
    Gui, 剪切板工具: Font, s9     ;//设置字体大小
    Loop, 19
    {
      i:=A_Index, v:=(i=16 ? "<<" : i=19 ? ">>" : i=18?"清空":"")
      j:=(i=1 ? "w250 Left" : i=16 ? "xm w50"
        : i=17 ? "x+0 w75" : i=18 ? "x+0 w75" :i=19 ? "x+0 w50" : "y+0 wp Left")
      Gui, 剪切板工具: Add, Button, %j% vbt%i% Hwndid -Wrap, %v%
      GuiControl, 剪切板工具: +g, %id%, % Run
    }
    Gui, 剪切板工具: Add,Checkbox, vdelspace gsubmit y+5 xs,去除空格
    Gui, 剪切板工具: Add,Checkbox, vdelEnter gsubmit x+5,去除空行
    Gui, 剪切板工具: Add,Button,  g合并 x+5,合并
    Gui, 剪切板工具: Add,Button,  g逆序 x+5,逆序
    Gui, 剪切板工具: Add,Checkbox,v编辑模式 gsubmit y+5 xs,编辑模式
    Gui, 剪切板工具: Add,Checkbox,checked v开启快捷键 gsubmit x+5,快捷键
    ; Gui, 剪切板工具: Show, NA, %A_ThisFunc%             ;首次加载,取消该代码不显示
    OnMessage(0x201, Func(A_ThisFunc).Bind("","Move"))  ;这句应该是用来绑定软件自己鼠标点击的
    v:=Func(A_ThisFunc).Bind("","")
  }
  Loop, 15
  {
    i:=A_Index, v:=MyClipData[(page-1)*15+i]
    j:=MyClipData.length()
    ; v:=(v="" ? v : "[" StrLen(v) "] " SubStr(v,1,50))       ;原来代码
    ; v:=(v="" ? v : j-((page-1)*10+i)+1 "-[" StrLen(v) "] " SubStr(v,1,50))    
    v:=(v="" ? v :"!" i "-[" j-((page-1)*15+i)+1 "] " SubStr(v,1,50))    
    v:=RegExReplace(v, "\s+", " ")
    GuiControl, 剪切板工具: , bt%i%, %v%
  }
  GuiControl, 剪切板工具: , bt17, % clear ? "删除模式":"粘贴模式"

  ; Gui, 剪切板工具: Show, NA          ;去掉这句话,则只在加载时显示窗口,之后复制不再显示窗口
}
Submit:
Gui, 剪切板工具:Submit,NoHide
if (delspace==1)
...
逆序:
ReversArray(MyClipData)
Loop, 10
{
  i:=A_Index, v:=MyClipData[(page-1)*10+i]
  ;;;;;;;; v:=(v="" ? v : "[" StrLen(v) "] " SubStr(v,1,50))       ;原来代码
  v:=(v="" ? v : i "-[" StrLen(v) "] " SubStr(v,1,50))    
  v:=RegExReplace(v, "\s+", " ")
  GuiControl, 剪切板工具: , bt%i%, %v%
}
return
...
#if (开启快捷键=1)
!1::
Clipboard:=MyClipData[(page-1)*10+1]
Sleep,20
Send ^v
return
!2::
Clipboard:=MyClipData[(page-1)*10+2]
Sleep,20
Send ^v
return
!3::
Clipboard:=MyClipData[(page-1)*10+3]
Sleep,20
Send ^v
return
!4::
Clipboard:=MyClipData[(page-1)*10+4]
Sleep,20
Send ^v
return
!5::
Clipboard:=MyClipData[(page-1)*10+5]
Sleep,20
Send ^v
return
!6::
Clipboard:=MyClipData[(page-1)*10+6]
Sleep,20
Send ^v
return
!7::
Clipboard:=MyClipData[(page-1)*10+7]
Sleep,20
Send ^v
return
!8::
Clipboard:=MyClipData[(page-1)*10+8]
Sleep,20
Send ^v
return
!9::
Clipboard:=MyClipData[(page-1)*10+9]
Sleep,20
Send ^v
return
!0::
Clipboard:=MyClipData[(page-1)*10+10]
Sleep,20
Send ^v
return
#if
...
if (OutputVar!="AutoHotkeyGUI")
{
  ToolTip
  old_control=
  return
}
WinGetTitle,OutputVarWin,ahk_id %OutputVarWin%
if(!InStr(OutputVarWin,"剪切板工具") )
{
  ToolTip
  old_control=
  return
}
if(OutputVarControl!=old_control)
{
ControlGetText, Outputtext , %OutputVarControl%, %OutputVarWin%
ToolTip,% Outputtext
old_control:=OutputVarControl
}
return
合并:
开始:=结束:=几条:=result := ""
InputBox,起始,哪一条开始,哪一条开始
InputBox,结束,合并到第几条,合并到第几条
几条:=结束-起始
if (几条>0 && 起始>0 && 结束>1)
{
Loop, %结束%
{
    if (A_Index > MyClipData.MaxIndex())
        break
    result .= MyClipData[A_Index]
}
; 将结果复制到剪贴板
clipboard := result
MyClipData.InsertAt(1,result), page:=1, clear:=0
  Loop, 10
  {
    i:=A_Index, v:=MyClipData[(page-1)*10+i]
    ;;;;;;;; v:=(v="" ? v : "[" StrLen(v) "] " SubStr(v,1,50))       ;原来代码
...
; # Original source code: https://github.com/Aatoz/AutoHotKey/blob/master/Lib/ini.ahk
; #############################################################################################################
; #############################################################################################################
; # Modified by dein0s
...
*/
	...
	Copy(SourceIni, bCopyFileName = true)
	{
		...
		if (IsObject(this))
		{
			...
			this := ini(SourceIni.GetFileName(), sIniString)

			; Restore file name.
			this.EasyIni_ReservedFor_m_sFile := sOldFileName
		}
		else
			return ini(bCopyFileName ? SourceIni.GetFileName() : "", sIniString)
	...
	*/
	test()
	{
		if (FileExist(this.GetFileName()))
			this := ini(this.GetFileName()) ; else nothing to reload.
		return this
	}
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	Save(sSaveAs="", bWarnIfExist=false)
	{
		...
		for k, v in this.EasyIni_ReservedFor_TopComments
		{
			...
			; FileAppend, %sLastAddedLine%, %sFile%, UTF-8
			initext .= sLastAddedLine
			bIsFirstLine := false
		}
		...
		for section, aKeysToVals in this
		{
			...
			initext := Trim(initext,"`n") "`n"
			initext := RegExReplace(initext, "\s{2,}(?=[^\[])", "`n")
			file := FileOpen(sFile, "w", "UTF-8"), file.Write(initext), file.Close()
			; --- MODIFICATION END (dein0s) ---
		}
		return true
	}
