...
Label_ScriptSetting: ; 脚本前参数设置
	...
	#SingleInstance Ignore							;允许多例运行，通过Single_Run限制单例
...
Label_AutoRun: ; 开机自启
	...
	Auto_Launch_reg := Auto_Launch_reg=A_ScriptDir "\" OutNameNoExt ".ahk" ? 1 : 0
	If(Auto_Launch!=Auto_Launch_reg){
		Auto_Launch_reg:=Auto_Launch
		If(Auto_Launch){
			RegWrite, REG_SZ, HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run, %OutNameNoExt%, %A_ScriptDir%\%OutNameNoExt%.ahk
		}Else{
...
;-----------------------------------【辅助函数功能】-----------------------------------------------
GetVersionComp(Version){ ; 获取版本字符串
...
; 来源： https://www.autoahk.com/archives/16443
GetCaret(Byref CaretX="", Byref CaretY="") { ; 获取输入光标位置
	static init
