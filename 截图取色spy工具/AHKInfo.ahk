...
if !(A_IsAdmin || InStr(DllCall("GetCommandLine", "str"), ".exe"" /r"))
	Try RunWait % "*RunAs " (s:=A_IsCompiled ? "" : A_AhkPath " /r ") """" A_ScriptFullPath """" (s ? "" : " /r")

; 230815 dbgba：调整了界面宽度、优化对64位系统的支持、对界面与功能细节进行优化和调整

;AHK版本:  AutoHotkey_L 1.1.09.02
#NoTrayIcon
;定义标题,在多处用到的!还是定义个变量比较好!后期修改时只改这里就行了
AHKInfo_Title=AHKInfo 1.3.7
;********************************************
;图标数据
...
ListView1_Text=窗口标题|窗口类|窗口ID|坐标|大小|相对坐标|样式|扩展样式|进程PID|进程名|进程路径|控件名|全局坐标|颜色 RGB|颜色 BGR　
...
Gui, 1:Add, Edit,HScroll ReadOnly x10 y75 w%控件宽% h162 vVisible_text,这里将显示的是可见文本`n`nDetectHiddenText Off`nWinGetText, 可见文本变量, ahk_class WinTitle
Gui, 1:Add, Edit,HScroll ReadOnly x10 y240 w%控件宽% h162 vHidden_text,这里将显示的是全部文本`n`nDetectHiddenText On`nWinGetText, 全部文本变量, ahk_class WinTitle
;==========================================================
...
WM_LBUTTONDBLCLK(wParam, lParam) {
  Static TempGuiControl
  If (A_GuiControl != "Generate_text") and (A_GuiControl != "Title")
    Return
  TempGuiControl := A_GuiControl
  SetTimer 延时获取整行, -50
  Return
  
  延时获取整行:
  SendInput {LButton Up}{LButton Up}
  if (TempGuiControl="Generate_text") {
    Send {Home}
    Sleep 50
    Send +{Down}
  } else
...
Generate(){ ;生成简单代码
  ...
  text4 := "x" StrSplit(text4, ", ")[1] " y" StrSplit(text4, ", ")[2]
  Gui, 1:ListView,ListView2
  ...
  SetText("",0)
	if (WinTitle!="" and WinTitle!="ahk_class") {
		...
		SetText(text)
		; 控件选中(选择框或单选框) Check替换为Uncheck即取消选中
		if text5!=
		{
			_ := InStr(text5,"Button") ? "; 点击控件 或 选中" : "; 点击控件" 
			text=
			(
			%_%
...
;===========================================
  ...
  GuiControl,,Title,%OutWin1% ahk_class %OutWin2%
  OutWin4=%WinX%, %WinY%
  OutWin5=%WinW%, %WinH%
  OutWin6_X:=OutX-WinX, OutWin6_Y:=OutY-WinY
  OutWin6=%OutWin6_X%, %OutWin6_Y%
  ...
  OutWin12 := OutCtrl1  ; 231228将透明色改成显示控件类名
  OutWin13=%OutX%, %OutY%
...
IsMenuItemChecked( MenuPos, SubMenuPos, hWnd ) { ; 检查菜单项是否选中,返回1或0
  hMenu :=DllCall("GetMenu", "UInt",hWnd ), hSubMenu := DllCall("GetSubMenu", "UInt",hMenu, "Int",MenuPos ), VarSetCapacity(mii, size := 4*4 + A_PtrSize*8, 0), NumPut(size, mii, 0), NumPut(1, mii, 4), DllCall( "GetMenuItemInfo", "UInt",hSubMenu, "UInt",SubMenuPos, "Int", 1, "UInt",&mii )
