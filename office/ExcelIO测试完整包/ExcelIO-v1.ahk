;#DllLoad "libxl.dll"
hModule := DllCall("LoadLibrary", "Str", "libxl.dll", "Ptr") 
;hModule := DllCall("LoadLibrary", "Str", "libxl64.dll", "Ptr") 
;hModule := DllCall("LoadLibrary" "x64\libxl.dll", "Str", "libxl.dll", "Ptr")  ; 避免了在循环中每次都需要使用 DllCall() 装载库.
;hModule := DllCall("loadlibrary" "x64\libxl64.dll", "Str", "libxl64.dll", "Ptr")  ; 避免了在循环中每次都需要使用 DllCall() 装载库.
;hModule := DllCall("loadlibrary x86\libxl.dll", "Str", "loadlibrary x64\libxl.dll", "Ptr") 
;hModule := DllCall("LoadLibrary" " x64\libxl64.dll", "Str", "libxl64.dll", "Ptr") 
;loadlibrary x86\libxl.dll
;loadlibrary x64\libxl.dll
...
class ExcelIO {
	...
	class IFormat extends ExcelIO.IBase {
		...
		setFont(font){
			return DllCall("libxl\xlFormatSetFont", "ptr", this.ptr, "ptr", font, "cdecl int")
		}
		numFormat(){
	...
	class IRichString extends ExcelIO.IBase {
		...
		addText(text, font := 0){
			return DllCall("libxl\xlRichStringAddText", "ptr", this.ptr, "str", text, "ptr", font, "cdecl")
		} 
	...
	class ISheet extends ExcelIO.IBase {
		...
		setCellFormat(row, col, format){
			return DllCall("libxl\xlSheetSetCellFormat", "ptr", this.ptr, "int", row, "int", col, "ptr", format, "cdecl")
		}
		...
		writeStr(row, col, value, format := 0){
			return DllCall("libxl\xlSheetWriteStr", "ptr", this.ptr, "int", row, "int", col, "str", value, "ptr", format, "cdecl int")
		}
		...
		writeRichStr(row, col, richString, format := 0){
			return DllCall("libxl\xlSheetWriteRichStr", "ptr", this.ptr, "int", row, "int", col, "ptr", richString, "ptr", format, "cdecl int")
		}
		...
		writeNum(row, col, value, format := 0){
			return DllCall("libxl\xlSheetWriteNum", "ptr", this.ptr, "int", row, "int", col, "double", value, "ptr", format, "cdecl int")
		}
		...
		writeBool(row, col, value, format := 0){
			return DllCall("libxl\xlSheetWriteBool", "ptr", this.ptr, "int", row, "int", col, "int", value, "ptr", format, "cdecl int")
		}
		...
		writeBlank(row, col, format){
			return DllCall("libxl\xlSheetWriteBlank", "ptr", this.ptr, "int", row, "int", col, "ptr", format, "cdecl int")
		}
		...
		writeFormula(row, col, expr, format := 0){
			return DllCall("libxl\xlSheetWriteFormula", "ptr", this.ptr, "int", row, "int", col, "str", expr, "ptr", format, "cdecl int")
		}
		writeFormulaNum(row, col, expr, value, format := 0){
			return DllCall("libxl\xlSheetWriteFormulaNum", "ptr", this.ptr, "int", row, "int", col, "str", expr, "double", value, "ptr", format, "cdecl int")
		}
		writeFormulaStr(row, col, expr, value, format := 0){
			return DllCall("libxl\xlSheetWriteFormulaStr", "ptr", this.ptr, "int", row, "int", col, "str", expr, "str", value, "ptr", format, "cdecl int")
		}
		writeFormulaBool(row, col, expr, value, format := 0){
			return DllCall("libxl\xlSheetWriteFormulaBool", "ptr", this.ptr, "int", row, "int", col, "str", expr, "int", value, "ptr", format, "cdecl int")
		}
		readComment(row, col){
		...
		writeError(row, col, ErrorType, format := 0){
			return DllCall("libxl\xlSheetWriteError", "ptr", this.ptr, "int", row, "int", col, "int", ErrorType, "ptr", format, "cdecl")
		}
		colWidth(col){
		...
		setCol(colFirst, colLast, width, format := 0, hidden := false){
			return DllCall("libxl\xlSheetSetCol", "ptr", this.ptr, "int", colFirst, "int", colLast, "double", width, "ptr", format, "int", hidden, "cdecl int")
		}
		setRow(row, height, format := 0, hidden := false){
			return DllCall("libxl\xlSheetSetRow", "ptr", this.ptr, "int", row, "double", height, "ptr", format, "int", hidden, "cdecl int")
		}
		rowHidden(row){
